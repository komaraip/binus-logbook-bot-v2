...
## 🚀 Deploying to cPanel or Production Server

### Requirements for Server Deployment

1. **Node.js** installed on your hosting (v16+)
2. **SSH access** to your cPanel/server
3. **Sufficient memory** (at least 1GB RAM recommended)

### Step-by-Step Deployment to cPanel

#### 1. Upload Files

Upload all project files to your cPanel via:
- **File Manager**, or
- **FTP/SFTP**, or
- **Git** (if available)
...
```env
# IMPORTANT: Set to production for server deployment
NODE_ENV=production
...
Playwright needs additional system dependencies on Linux servers:

```bash
# Install Playwright browsers
npx playwright install chromium

# Install system dependencies (if you have sudo access)
npx playwright install-deps chromium
```

If you don't have sudo access, contact your hosting provider to install these packages:
- `libnss3`, `libatk1.0-0`, `libatk-bridge2.0-0`, `libcups2`, `libxkbcommon0`, `libxcomposite1`, `libxrandr2`, `libgbm1`, `libpango-1.0-0`, `libasound2`
...
```
http://yourdomain.com/binuslogbookbot
```

### Important Notes for Production

- ✅ **Headless Mode**: The bot runs WITHOUT showing a browser window (this is normal!)
- ✅ **Background Processing**: All automation happens server-side
- ✅ **Monitor Logs**: Use `pm2 logs` or check console output to monitor progress
- ✅ **Session Persistence**: Sessions are saved in `data/session/` folder
- ⚠️ **Memory Usage**: The bot uses ~200-500MB RAM during operation
- ⚠️ **Security**: Protect your `.env` file and don't expose it publicly

### Troubleshooting Server Deployment

**Browser launch failed on server:**
```bash
